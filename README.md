...
| 🔌 **开放 API** | API Key 管理 · 外部创建邮箱 · 获取最新验证码 · 自动选择域名 |
...
管理后台已集成 **External API / API Keys** 模块，可创建 API Key 后供脚本或程序调用。
...
`POST /api/ext/accounts` 已支持自动选域名：
- 传 `address`：使用完整邮箱地址
- 传 `local + domain`：使用指定前缀与域名
- 只传 `local`：自动从 `MAIL_DOMAIN` 中随机选择域名
- 不传参数：自动生成随机前缀并随机选择域名

详细说明见 [API 文档](docs/api.md)。
